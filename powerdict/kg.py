...
__all__ = ['load_entity_data', 'get_entity_url', 'load_raw_triplets_df', 'load_build_data', 'aggregate_triplets',
           'extract_entity_attribute_triplets', 'extract_cleaned_triplets', 'construct_knowledge_graph_triplets']
...
def load_entity_data(
    entity_data_fp: str='data/intermediate/site_data.json'
):
...
# Cell
get_entity_url = lambda entity_id, endpoint='https://osuked.github.io/Power-Station-Dictionary/objects': f'{endpoint}/{entity_id}'

# Cell
def load_raw_triplets_df(
    entity_data: dict
):
    triplet_data = []
        ...
        if 'attributes' in data.keys():
            for attr in data['attributes']:
                if 'rdfType' in attr['field_schema'].keys():
                    triplet_data += [[get_entity_url(entity_id), attr['field_schema']['rdfType'], attr['value'], attr['source']]]
...
def aggregate_triplets(
    ...
    math_agg_options: list=['mean', 'max', 'min', 'sum', 'std']
):
    ...
    if aggregation == False:
        ...
        agg_triplets = list(df_triplets_agg.drop(columns=['index'])[['subject', 'predicate', 'object']].to_records(index=False))
...
def extract_entity_attribute_triplets(
    ...
    for source in source_priority_list:
        ...
        if df_triplets_entity_attribute_source.shape[0] > 0:
            entity_attribute_triplets += aggregate_triplets(df_triplets_entity_attribute_source, aggregation=aggregation)

    return entity_attribute_triplets

def extract_cleaned_triplets(
    df_triplets: pd.DataFrame,
    build_data: dict
):
    cleaned_triplets = []
